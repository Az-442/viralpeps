...
export default function ToolsPage() {
  const [recon, setRecon] = useState({ peptideMg: 5, waterMl: 1, doseMcg: 250 });
  const [reconResult, setReconResult] = useState<null | { unitsPerDose: number; mcgPerUnit: number }>(null);

  const [convert, setConvert] = useState({ value: 100, from: 'mcg', to: 'mg' });
  ...
  const calcRecon = () => {
    ...
    const mcgPerUnit = mcgPerMl / 100; // 1ml = 100 units on insulin syringe
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-6xl mx-auto px-4 py-10 grid md:grid-cols-3 gap-6">
        ...
        <div className="bg-white border border-gray-100 rounded-xl p-6 shadow-sm">
          ...
          <p className="text-xs text-gray-500 mb-5">Calculate how much BAC water to add for your target dose.</p>

          <div className="space-y-3">
            <div>
              <label className="text-xs text-gray-600 font-medium block mb-1">Peptide amount (mg)</label>
              <input type="number" value={recon.peptideMg} onChange={e => setRecon({...recon, peptideMg: parseFloat(e.target.value) || 0})} className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm outline-none focus:border-blue-500" />
            </div>
            <div>
              <label className="text-xs text-gray-600 font-medium block mb-1">BAC water (ml)</label>
              <input type="number" value={recon.waterMl} onChange={e => setRecon({...recon, waterMl: parseFloat(e.target.value) || 0})} step="0.1" className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm outline-none focus:border-blue-500" />
            </div>
            <div>
              <label className="text-xs text-gray-600 font-medium block mb-1">Target dose (mcg)</label>
              <input type="number" value={recon.doseMcg} onChange={e => setRecon({...recon, doseMcg: parseFloat(e.target.value) || 0})} className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm outline-none focus:border-blue-500" />
            </div>
            ...
            {reconResult && (
              <div className="bg-blue-50 border border-blue-100 rounded-lg p-3 text-sm space-y-1">
                ...
                <p className="text-gray-500 text-xs">{reconResult.mcgPerUnit} mcg per unit</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="bg-white border border-gray-100 rounded-xl p-6 shadow-sm">
          ...
          <div className="space-y-3">
            <div>
              <label className="text-xs text-gray-600 font-medium block mb-1">Value</label>
              <input type="number" value={convert.value} onChange={e => setConvert({...convert, value: parseFloat(e.target.value) || 0})} className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm outline-none focus:border-blue-500" />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-xs text-gray-600 font-medium block mb-1">From</label>
                <select value={convert.from} onChange={e => setConvert({...convert, from: e.target.value})} className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm outline-none focus:border-blue-500 bg-white">
              ...
              <div>
                ...
                <select value={convert.to} onChange={e => setConvert({...convert, to: e.target.value})} className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm outline-none focus:border-blue-500 bg-white">
                  <option value="mcg">mcg (µg)</option>
                  <option value="mg">mg</option>
                  <option value="g">g</option>
                </select>
        ...
        <div className="bg-white border border-gray-100 rounded-xl p-6 shadow-sm">
          ...
          <p className="text-xs text-gray-500 mb-5">Look up the half-life of common research peptides.</p>

          <div className="space-y-3">
            <div>
              <label className="text-xs text-gray-600 font-medium block mb-1">Peptide</label>
              <select value={half.substance} onChange={e => setHalf({...half, substance: e.target.value})} className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm outline-none focus:border-blue-500 bg-white">
            ...
            {half.substance === 'Custom' && (
              <div>
                <label className="text-xs text-gray-600 font-medium block mb-1">Enter half-life</label>
                <input type="text" value={half.customHalfLife} onChange={e => setHalf({...half, customHalfLife: e.target.value})} placeholder="e.g. 2 hours" className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm outline-none focus:border-blue-500" />
              </div>
            )}
